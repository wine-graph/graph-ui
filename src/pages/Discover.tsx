            ...
            {loading ? (
                ...
                <>
                    ...
                    <nav className="mb-4 flex items-center gap-2 text-sm" aria-label="Breadcrumb">
                        <CrumbButton onClick={resetToCountries} active={showCountries}>Countries</CrumbButton>
                        {selectedCountry ? (
                            <>
                                <span className="text-gray-400">/</span>
                                <CrumbButton onClick={resetToRegions}
                                             active={showRegions}>{selectedCountry.name}</CrumbButton>
                            </>
                        ) : null}
                        {selectedRegion ? (
                            <>
                                <span className="text-gray-400">/</span>
                                <CrumbButton active={showAreas}>{selectedRegion.name}</CrumbButton>
                            </>
                        ) : null}
                    </nav>
                    ...
                    <Grid>
                        ...
                        {showRegions && selectedCountry?.regions.map(r => (
                            <DomainCard
                                key={r.id}
                                title={r.name}
                                desc={r.description}
                        ...
                        {showAreas && selectedRegion?.areas.map(a => (
                            <DomainCard
                                key={a.id}
                                title={a.name}
                                desc={a.description}
                                onClick={() => {/* Could navigate deeper in future */
                                }}
                            />
                        ))}
                    </Grid>
