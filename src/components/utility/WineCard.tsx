import { FaRegHeart, FaRegEye } from "../../assets/icons";
import Button from "./Button";
import type { Wine } from "../../types/Wine";

const WineCard: React.FC<Wine> = ({
  name,
  pricePerBottle,
  vintage,
  producer,
  ...
  return (
    <div className="border border-border rounded-2xl  p-4 shadow flex flex-col justify-between">
      ...
      <div className="action w-full flex items-center justify-between mt-7">
        <Button className=" border-[#c4a1a8] px-3 md:px-4 text-sm md:text-base hover:bg-[#ffede1]/50 font-medium text-textPrimary">
          <FaRegEye className="text-textPrimary-1" />
          <span className="text-textPrimary-1">View</span>
        </Button>
        <Button className="bg-primary text-white hover:bg-buttonHover px-3 md:px-4 text-sm md:text-base border border-primary hover:border-buttonHover font-medium">
          <FaRegHeart />
          <span>Save</span>
        </Button>
      </div>
